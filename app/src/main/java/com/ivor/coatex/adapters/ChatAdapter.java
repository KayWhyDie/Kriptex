package com.ivor.kriptex.adapters;
...
import com.ivor.kriptex.VideoViewActivity;
...
//        data.da
    ...
    private Drawable applyTint(Drawable icon) {
        icon.setColorFilter(
                new PorterDuffColorFilter(ContextCompat.getColor(mChatActivity, R.color.colorGrey), PorterDuff.Mode.SRC_IN)
        );
        return icon;
    }
    ...
    @Override
    public void onBindViewHolder(ChatHolderText holder, int position) {
        ...
        if (message.getType() == Message.TYPE_IMAGE
            ...
            if (tx) {
                ...
                if (checkFileDownloaded(message)) {
                    if (message.getType() != Message.TYPE_FILE) { // File does not have any image
            ...
            if (message.getQuotedMessageId() != null) {
                ...
                if (d != null) {
                    ...
                    if (d.getType() == Message.TYPE_IMAGE || d.getType() == Message.TYPE_VIDEO) {
                        ...
                        if (file.exists()) { // check if file exists then load it
                            loadIntoImageView(d, holder.imvwReply);
                        } else { // otherwise load the thumbnail
        ...
        if (mSearchText != null) {
            setHighLightedText(holder.content, mSearchText);
//            mSearchSet.add(position);
        }

        /**
         * writing here cuz above here text is being set into message {@link TextView}
         */
        if (message.getType() == Message.TYPE_FILE) {
            holder.content.setVisibility(View.VISIBLE);
    ...
    /**
     * loads the image into {@link ImageView} by checking the {@link Message}.getType
     * if its video, it will load the thumbnail, otherwise the image.
     *
     * @param message
     * @param imageView
     */
    private void loadIntoImageView(Message message, ImageView imageView) {
        ...
        Glide.with(mChatActivity).load(Uri.fromFile(new File(filePath))).into(imageView);
    }
