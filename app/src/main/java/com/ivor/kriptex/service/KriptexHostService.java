...
public class KriptexHostService extends Service {

    private String TAG = "KriptexHostService";
    ...
    public void update() {
        ...
        // If Tor failed to start (e.g., wrong ABI) or exited, show the raw status.
        if (status != null) {
            String trimmed = status.trim();
            if (trimmed.startsWith("Tor start ") || trimmed.startsWith("Tor exited")) {
                startForeground(trimmed, -1);
                return;
            }
        }
        ...
        if (status != null && status.contains("%") && status.length() > prefix.length() && status.startsWith(prefix)) {
